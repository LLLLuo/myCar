...
typedef struct Motor//-------------------------------------------------速度
{
  ...
  int16 RealSpeed;  // 编码器读取的速度
  int16 AveSpeed;  //平均速度
  int16 RealSpeedSave[10]; //存储速度 
  int16 SpeedError; //速度误差
  int16 SpeedErrorSave[3];  //存放速度误差
  int16 MotorOutputDuty; //输出占空比

  
  
 // float TrackComplexity; //ABS(CurrentError*10)
 // float ProspectSee;  //ABS(DoubleError*100)
  int16 TargetSpeed;//预设速度
